...
#include <ns3/fusion-agent.h>
#include <ns3/fusion-gym-env.h>
#ifdef NS3_OPENGYM
...
// Fusion Gym configuration
bool enable_gym = false;
uint32_t gym_port = 5555;
Ptr<FusionAgent> global_fusion_agent;
Ptr<FusionGymEnv> global_gym_env;
...
int main(int argc, char *argv[])
{
...
#endif
	{
		...
		while (!conf.eof())
		{
			...
			}else if (key.compare("ENABLE_GYM") == 0){
				uint32_t v;
				conf >> v;
				enable_gym = (v != 0);
				std::cout << "ENABLE_GYM\t\t\t\t" << enable_gym << '\n';
			}else if (key.compare("GYM_PORT") == 0){
				conf >> gym_port;
				std::cout << "GYM_PORT\t\t\t\t" << gym_port << '\n';
			}
			fflush(stdout);
		}
		conf.close();
	...
	else if (cc_mode == 11) // fusion, no extra header
	...
	// Initialize Fusion Gym Environment (if enabled and using Fusion mode)
	if (cc_mode == 11) {
		// Always create Fusion Agent for Fusion mode
		global_fusion_agent = CreateObject<FusionAgent>();
		global_fusion_agent->Initialize();
		
		if (enable_gym) {
#ifdef NS3_OPENGYM
			std::cout << "Initializing Fusion Gym Environment on port " << gym_port << std::endl;
			
			// Create Fusion Gym Environment (inherits from OpenGymInterface)
			global_gym_env = CreateObject<FusionGymEnv>();
			global_gym_env->SetPort(gym_port);
			global_gym_env->SetFusionAgent(global_fusion_agent);
			global_fusion_agent->SetGymEnv(global_gym_env);
			global_fusion_agent->SetUseGym(true);
			
			std::cout << "Fusion Gym Environment initialized successfully on port " << gym_port << std::endl;
			std::cout << "Waiting for Python client to connect..." << std::endl;
#else
			std::cout << "Warning: OpenGym not available. Gym mode disabled." << std::endl;
			std::cout << "Please install ns3-gym and ensure opengym module is available." << std::endl;
			std::cout << "Run install_opengym.sh (Linux/Mac) or install_opengym.bat (Windows) to install the opengym module." << std::endl;
			enable_gym = false;
			global_fusion_agent->SetUseGym(false);
#endif
		} else {
			// Fusion mode without gym - use heuristic
			std::cout << "Fusion mode enabled without Gym. Using heuristic parameter generation." << std::endl;
			global_fusion_agent->SetUseGym(false);
		}
	}
	...
	for (uint32_t i = 0; i < node_num; i++){
		if (n.Get(i)->GetNodeType() == 0){ // is server
			...
			// Set Fusion Agent if using Fusion mode
			if (cc_mode == 11 && global_fusion_agent) {
				rdmaHw->SetFusionAgent(global_fusion_agent);
			}
