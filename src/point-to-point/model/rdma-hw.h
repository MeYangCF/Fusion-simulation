...
public:
	 ...
	 * Fusion
	 ********************/
	Ptr<FusionAgent> m_fusionAgent;  // Fusion Agent for RL
	
	void SetFusionAgent(Ptr<FusionAgent> agent);
	Ptr<FusionAgent> GetFusionAgent() const;
	
	void HandleAckFusion(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch);
	void UpdateRateFusion(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch);
	void FusionRateDecrease(Ptr<RdmaQueuePair> qp);  // CNP received
	void FusionFastRecovery(Ptr<RdmaQueuePair> qp);  // T < F && BC < F
	void FusionActiveIncrease(Ptr<RdmaQueuePair> qp); // T >= F || BC >= F
	void FusionHyperIncrease(Ptr<RdmaQueuePair> qp);  // T >= F && BC >= F
	void ScheduleFusionUpdate(Ptr<RdmaQueuePair> qp); // Schedule timer updates
	void FusionTimerIncrement(Ptr<RdmaQueuePair> qp);  // Timer increment callback
};
