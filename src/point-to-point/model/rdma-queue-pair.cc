...
RdmaQueuePair::RdmaQueuePair(uint16_t pg, Ipv4Address _sip, Ipv4Address _dip, uint16_t _sport, uint16_t _dport){
	...
	fusion.mu2 = 3.0;
	fusion.mu3 = 8.0;
	fusion.m_targetRate = DataRate(0);
	fusion.m_curRate = DataRate(0);
	fusion.m_refRate = DataRate(0);
	fusion.m_lastRtt = 0;
	fusion.m_minRtt = 0;
	fusion.m_alpha = 0.0;
	fusion.m_timer = 0;
	fusion.m_byteCounter = 0;
	fusion.m_byteCounterAccumulator = 0;
	fusion.m_timerEvent = EventId();
	fusion.m_lastUpdateSeq = 0;
	fusion.m_lastUpdateTime = Time(0);
	fusion.m_cnpReceived = false;
	fusion.m_ecnMarkedPackets = 0;
	fusion.m_totalPackets = 0;
	fusion.m_nackCount = 0;
	fusion.m_packetsSent = 0;
	fusion.m_packetsAcked = 0;
	fusion.m_lastStatsReset = Time(0);
}
