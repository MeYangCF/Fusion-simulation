...
void RdmaHw::AddQueuePair(uint64_t size, uint16_t pg, Ipv4Address sip, Ipv4Address dip, uint16_t sport, uint16_t dport, uint32_t win, uint64_t baseRtt, Callback<void> notifyAppFinish){
	...
	if (m_cc_mode == 1){
		...
		qp->fusion.m_curRate = m_bps;
		qp->fusion.m_targetRate = m_bps;
		qp->fusion.m_refRate = m_bps;
		qp->fusion.m_lastRtt = 0;
		qp->fusion.m_minRtt = 0;
		qp->fusion.m_timer = 0;
		qp->fusion.m_byteCounter = 0;
		qp->fusion.m_byteCounterAccumulator = 0;
		qp->fusion.m_timerEvent = EventId();
		qp->fusion.m_alpha = 0.0;
		qp->fusion.m_cnpReceived = false;
		...
		qp->fusion.m_lastStatsReset = Time(0);
		qp->fusion.delta = 1.0;
		qp->fusion.gamma = 1.0;
		qp->fusion.beta = 0.1;
		qp->fusion.mu1 = 2.5;
		qp->fusion.mu2 = 3.0;
		qp->fusion.mu3 = 8.0;
		ScheduleFusionUpdate(qp);
	}

	m_nic[nic_idx].dev->NewQp(qp);
	if (m_cc_mode == 11 && m_fusionAgent) {
		m_fusionAgent->StartMonitoring(qp);
	}
}
...
int RdmaHw::ReceiveCnp(Ptr<Packet> p, CustomHeader &ch){
	...
	if (qp->m_rate == 0)			//lazy initialization	
	{
		...
		if (m_cc_mode == 1){
			...
			qp->fusion.m_curRate = dev->GetDataRate();
			qp->fusion.m_targetRate = dev->GetDataRate();
			qp->fusion.m_refRate = dev->GetDataRate();
			if (!qp->fusion.m_timerEvent.IsRunning()) {
				ScheduleFusionUpdate(qp);
			}
		}
	}
	...
	if (m_cc_mode == 11 && qp) {
		if (total > 0) {
			qp->fusion.m_ecnMarkedPackets += qfb;
			qp->fusion.m_totalPackets += total;
		}
	}
...
void RdmaHw::PktSent(Ptr<RdmaQueuePair> qp, Ptr<Packet> pkt, Time interframeGap){
	...
	if (m_cc_mode == 11) {
		uint32_t pktBytes = pkt->GetSize();
		qp->fusion.m_byteCounterAccumulator += pktBytes;
		qp->fusion.m_packetsSent++;
		double gamma = qp->fusion.gamma;
		uint64_t R_C_bps = qp->fusion.m_curRate.GetBitRate();
		if (R_C_bps == 0) R_C_bps = qp->m_rate.GetBitRate();
		uint64_t BC_a_bytes = (uint64_t)(gamma * R_C_bps / 8);
		if (BC_a_bytes > 0 && qp->fusion.m_byteCounterAccumulator >= BC_a_bytes) {
			qp->fusion.m_byteCounter++;
			qp->fusion.m_byteCounterAccumulator = 0;
		}
	}
}
...
void RdmaHw::HandleAckFusion(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch)
{
	uint32_t ack_seq = ch.ack.seq;
	uint8_t cnp = (ch.ack.flags >> qbbHeader::FLAG_CNP) & 1;
	uint64_t currentRtt = qp->m_baseRtt;
	if (ch.ack.ih.nhop > 0) {
		uint32_t qlen = ch.ack.ih.hop[ch.ack.ih.nhop-1].GetQlen();
		DataRate linkRate = m_nic[GetNicIdxOfQp(qp)].dev->GetDataRate();
		if (linkRate.GetBitRate() > 0) {
			uint64_t queuingDelay = (uint64_t)(qlen * 8 * m_mtu * 1e9 / linkRate.GetBitRate());
			currentRtt = qp->m_baseRtt + queuingDelay;
		} else {
			currentRtt = qp->m_baseRtt;
		}
	}
	if (currentRtt < qp->m_baseRtt) {
		currentRtt = qp->m_baseRtt;
	}
	if (qp->fusion.m_minRtt == 0 || currentRtt < qp->fusion.m_minRtt) {
		qp->fusion.m_minRtt = currentRtt;
	}
	if (qp->fusion.m_minRtt < qp->m_baseRtt) {
		qp->fusion.m_minRtt = qp->m_baseRtt;
	}
	qp->fusion.m_lastRtt = currentRtt;
	if (ch.l3Prot == 0xFD) {
		qp->fusion.m_nackCount++;
	}
	qp->fusion.m_packetsAcked++;
	if (cnp) {
		qp->fusion.m_cnpReceived = true;
		FusionRateDecrease(qp);
	} else {
		qp->fusion.m_cnpReceived = false;
	}
	UpdateRateFusion(qp, p, ch);
}

void RdmaHw::UpdateRateFusion(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch)
{
	uint64_t d_q = 0;
	if (qp->fusion.m_lastRtt > qp->fusion.m_minRtt) {
		d_q = qp->fusion.m_lastRtt - qp->fusion.m_minRtt;
	}
	if (d_q == 0) d_q = 1;
	double delta = std::max(0.5, std::min(1.5, qp->fusion.delta));
	double d_q_sec = d_q * 1e-9;
	double ref_rate_bps = 1.0 / (delta * d_q_sec);
	DataRate linkRate = m_nic[GetNicIdxOfQp(qp)].dev->GetDataRate();
	uint64_t minRefRate = 1000000;
	uint64_t maxRefRate = linkRate.GetBitRate();
	if (ref_rate_bps < minRefRate) {
		ref_rate_bps = minRefRate;
	} else if (ref_rate_bps > maxRefRate) {
		ref_rate_bps = maxRefRate;
	}
	DataRate R_REF = DataRate((uint64_t)ref_rate_bps);
	qp->fusion.m_refRate = R_REF;
	uint32_t F = m_rpgThreshold;
	if (qp->fusion.m_timer < F && qp->fusion.m_byteCounter < F) {
		FusionFastRecovery(qp);
	} else if (qp->fusion.m_timer >= F || qp->fusion.m_byteCounter >= F) {
		if (qp->fusion.m_timer >= F && qp->fusion.m_byteCounter >= F) {
			FusionHyperIncrease(qp);
		} else {
			FusionActiveIncrease(qp);
		}
	}
	qp->m_rate = qp->fusion.m_curRate;
...
void RdmaHw::FusionRateDecrease(Ptr<RdmaQueuePair> qp)
{
	DataRate R_REF = qp->fusion.m_refRate;
	DataRate R_C = qp->fusion.m_curRate;
	double mu1 = std::max(2.1, std::min(4.0, qp->fusion.mu1));
	double mu2 = std::max(mu1, std::min(4.0, qp->fusion.mu2));
	if (R_C.GetBitRate() < 8 * R_REF.GetBitRate()) {
		qp->fusion.m_targetRate = R_C;
		uint64_t newCurRate = (uint64_t)(R_C.GetBitRate() * (1 - qp->fusion.m_alpha / 2));
		if (newCurRate < m_minRate.GetBitRate()) {
			newCurRate = m_minRate.GetBitRate();
		}
		qp->fusion.m_curRate = DataRate(newCurRate);
	} else {
		uint64_t newTargetRate = (uint64_t)(R_C.GetBitRate() / mu1);
		uint64_t newCurRate = (uint64_t)(R_C.GetBitRate() / mu2);
		if (newTargetRate < m_minRate.GetBitRate()) {
			newTargetRate = m_minRate.GetBitRate();
		}
		if (newCurRate < m_minRate.GetBitRate()) {
			newCurRate = m_minRate.GetBitRate();
		}
		qp->fusion.m_targetRate = DataRate(newTargetRate);
		qp->fusion.m_curRate = DataRate(newCurRate);
	}
	qp->fusion.m_alpha = (1 - m_g) * qp->fusion.m_alpha + m_g;
	qp->fusion.m_timer = 0;
	qp->fusion.m_byteCounter = 0;
	qp->fusion.m_byteCounterAccumulator = 0;
	if (qp->fusion.m_timerEvent.IsRunning()) {
		Simulator::Cancel(qp->fusion.m_timerEvent);
	}
	ScheduleFusionUpdate(qp);
}

void RdmaHw::FusionFastRecovery(Ptr<RdmaQueuePair> qp)
{
	DataRate R_REF = qp->fusion.m_refRate;
	DataRate R_C = qp->fusion.m_curRate;
	double mu3 = std::max(6.0, std::min(10.0, qp->fusion.mu3));
	if (R_C.GetBitRate() < 3 * R_REF.GetBitRate()) {
	} else {
		int64_t decrease = (int64_t)(R_C.GetBitRate() / mu3);
		int64_t currentTarget = (int64_t)qp->fusion.m_targetRate.GetBitRate();
		int64_t newTargetRate = currentTarget - decrease;
		if (newTargetRate < (int64_t)m_minRate.GetBitRate()) {
			newTargetRate = (int64_t)m_minRate.GetBitRate();
		}
		if (newTargetRate < 0) {
			newTargetRate = (int64_t)m_minRate.GetBitRate();
		}
		qp->fusion.m_targetRate = DataRate((uint64_t)newTargetRate);
	}
	qp->fusion.m_curRate = DataRate((uint64_t)((R_C.GetBitRate() + qp->fusion.m_targetRate.GetBitRate()) / 2));
}

void RdmaHw::FusionActiveIncrease(Ptr<RdmaQueuePair> qp)
{
	DataRate R_REF = qp->fusion.m_refRate;
	DataRate R_C = qp->fusion.m_curRate;
	DataRate R_L = m_nic[GetNicIdxOfQp(qp)].dev->GetDataRate();
	double beta = std::max(0.01, std::min(0.99, qp->fusion.beta));
	double mu3 = std::max(6.0, std::min(10.0, qp->fusion.mu3));
	if (R_C.GetBitRate() < 3 * R_REF.GetBitRate()) {
		uint64_t increment = std::min((uint64_t)(0.1 * R_C.GetBitRate()), 
		                             (uint64_t)(beta * R_L.GetBitRate()));
		uint64_t newTargetRate = qp->fusion.m_targetRate.GetBitRate() + increment;
		if (newTargetRate > qp->m_max_rate.GetBitRate()) {
			newTargetRate = qp->m_max_rate.GetBitRate();
		}
		qp->fusion.m_targetRate = DataRate(newTargetRate);
	} else {
		int64_t decrease = (int64_t)(R_C.GetBitRate() / mu3);
...
void RdmaHw::FusionHyperIncrease(Ptr<RdmaQueuePair> qp)
{
	DataRate R_REF = qp->fusion.m_refRate;
	DataRate R_C = qp->fusion.m_curRate;
	double mu3 = std::max(6.0, std::min(10.0, qp->fusion.mu3));
	uint32_t F = m_rpgThreshold;
	uint32_t i = std::min(qp->fusion.m_byteCounter, qp->fusion.m_timer - F + 1);
	DataRate R_HAI = DataRate("100Mb/s");
	if (R_C.GetBitRate() < 3 * R_REF.GetBitRate()) {
		uint64_t newTargetRate = qp->fusion.m_targetRate.GetBitRate() + i * R_HAI.GetBitRate();
		if (newTargetRate > qp->m_max_rate.GetBitRate()) {
			newTargetRate = qp->m_max_rate.GetBitRate();
		}
		qp->fusion.m_targetRate = DataRate(newTargetRate);
...
void RdmaHw::ScheduleFusionUpdate(Ptr<RdmaQueuePair> qp)
{
	if (qp->fusion.m_timerEvent.IsRunning()) {
		Simulator::Cancel(qp->fusion.m_timerEvent);
	}
	double delta = qp->fusion.delta;
	delta = std::max(0.5, std::min(1.5, delta));
	double interval_us = delta * 55.0;
	if (interval_us < 1.0) {
		interval_us = 1.0;
	}
	Time timerInterval = MicroSeconds((uint64_t)interval_us);
	qp->fusion.m_timerEvent = Simulator::Schedule(timerInterval, 
	                                              &RdmaHw::FusionTimerIncrement, 
	                                              this, qp);
}

void RdmaHw::FusionTimerIncrement(Ptr<RdmaQueuePair> qp)
{
	if (m_cc_mode == 11 && qp && !qp->IsFinished()) {
		qp->fusion.m_timer++;
		ScheduleFusionUpdate(qp);
	}
}

void RdmaHw::SetFusionAgent(Ptr<FusionAgent> agent)
{
	m_fusionAgent = agent;
}

Ptr<FusionAgent> RdmaHw::GetFusionAgent() const
{
	return m_fusionAgent;
}
